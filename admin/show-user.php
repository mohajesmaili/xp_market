...
<html lang="en">
  <head>
    ...
    <title>Mr.Developer | show_user</title>
          ...
          <div id="sidebar"  class="nav-collapse ">
              ...
              <ul class="sidebar-menu" id="nav-accordion">
                  ...
                  <li class="sub-menu">
                      ...
                      <ul class="sub">
                          <li class="active"><a  href="show-user.php">نمایش کاربران</a></li>
          ...
          <section class="wrapper">
                  ...
                  <div class="col-md-12" style="margin-top: 20px;">
                      <div class="content-panel">
                          <h4></i>کاربران</h4><hr><table class="table table-striped table-advance table-hover">
                              <thead>
                              ...
                              <?php                 
                                require('connect.php');
                                $show_admins=("SELECT * FROM `admin`");
                                $result=mysqli_query($sql,$show_admins);
                                if(mysqli_num_rows($result) > 0){
                                for($i=1;$row=mysqli_fetch_assoc($result);$i++){
                                $name=$row["name"];
                                $count_post=("SELECT count(id) as count_post FROM `blog_post` WHERE admin='$name' ");
                                $result_count=mysqli_query($sql,$count_post);
                                $ftech_count=mysqli_fetch_assoc($result_count);

                                echo'<form method="post" action="inc/dladmin.php" id="frm"   onSubmit="return dl()">
                                      <tr style="font-family:roya;">
                                      <td>'.$row["name"].'</a></td>
                                      <td>'.$row["semat"].'</td>
                                      <td>'.$ftech_count["count_post"].'</td>
                                      <td>'.$row["permission"].'</td>
                                      <td><img src="../assets/img/admin/'.$row['id'].'.jpg" width="60" height="60" style="border-radius:8px;"/></td>
                                      <td>
                                      <a href="edit-pass.php?adminid='.$row['id'].'" class="btn btn-black btn-xs"><i class="fa fa-key"></i></a>
                                      <a href="edit-admin.php?adminid='.$row['id'].'" class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i></a>
