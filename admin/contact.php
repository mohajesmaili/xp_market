      ...
      <aside style="float: right;margin-right: 210px;">
          <div id="sidebar"  class="nav-collapse ">
              ...
              <ul class="sidebar-menu" id="nav-accordion">
                  ...
                  <li class="sub-menu">
                      <a class="active" href="contact.php?pageid=1" >
                      ...
                      <div class="content-panel">
                          <h4></i>نظرات کاربران</h4><hr><table class="table table-striped table-advance table-hover">
                              ...
                              <?php
                                ...
                                $show_pages=("SELECT * FROM `contact`");
                                $resu=mysqli_query($sql,$show_pages);
                                $coun = mysqli_num_rows($resu);

                                $contact=("SELECT * FROM `contact` ORDER BY id DESC");
                                $result=mysqli_query($sql,$contact);
                                if(mysqli_num_rows($result) > 0){
                                for($i=1;$row=mysqli_fetch_assoc($result);$i++){
                                echo'<form method="post" action="inc/dl-comment.php" id="frm"   onSubmit="return dl()">
                                      <tr>
                                      <td>'.$row["name"].'</a></td>
                                    ...
                                    if($row["vaziat"]==0){
                                        echo'
                                        <a href="inc/confirm_c.php?id='.$row["id"].'&vaziat='.$row["vaziat"].'&page_id_hidden='.$page.'" style="margin-right:0" class="btn btn-danger btn-xs" name="change" ><i class="fa fa-lock" ></i></a>';
                                    }else if($row["vaziat"]==1){
                                        echo'
                                        <a href="inc/confirm_c.php?id='.$row["id"].'&vaziat='.$row["vaziat"].'&page_id_hidden='.$page.'" style="margin-right:0" class="btn btn-success btn-xs" name="change" ><i class="fa fa-unlock" ></i></a>';
                                    }
                                    echo'</td>
                                    </tr>
                                    </form>';
                          ...
                          <div style="align-items: center;direction: ltr;text-align: center;" class="general-pagination group">
                            ...
                            echo '<a style="margin-right:2px;" href="contact.php?pageid='.$i.'"><span class="badge bg-warning">'.$i.'</span></a>';
                             }
                            ?>
                          </div>
                      </div><!-- /content-panel -->
