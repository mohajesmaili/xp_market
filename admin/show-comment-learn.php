...
<html lang="en">
  <head>
    ...
    <title>Mr.Developer | show_comment_learn</title>
              ...
              <ul class="sidebar-menu" id="nav-accordion">
                  ...
                  <li class="sub-menu">
                      ...
                      <ul class="sub">
                          ...
                          <li class="active"><a  href="show-comment-learn.php?pageid=1">مشاهده نظرات</a></li>
                      </ul>
                  </li> 
                      ...
                      <div class="content-panel">
                          <h4></i>نظرات کاربران</h4><hr><table class="table table-striped table-advance table-hover">
                              ...
                              <?php                 
                                ...
                                $show_pages=("SELECT * FROM `comment_learn`");
                                $comment_learn=("SELECT *,comment_learn.id as cbi FROM `tutorails_learn`,`comment_learn` WHERE comment_learn.learnid=tutorails_learn.id ORDER BY comment_learn.id DESC LIMIT $start,$per_page");
                                $change=mysqli_query($sql,"SELECT * FROM `comment_learn` WHERE vaziat=0");
                                $result=mysqli_query($sql,$comment_learn);
                                $resu=mysqli_query($sql,$show_pages);
                                $coun = mysqli_num_rows($resu);
                                if(mysqli_num_rows($result) > 0){
                                for($i=1;$row=mysqli_fetch_assoc($result);$i++){
                                echo'<form method="post" action="inc/dlcomment_learn.php" id="frm"   onSubmit="return dl()">
                                      <tr>
                                      <td>'.$row["name"].'</a></td>
                                      ...
                                      if($row["vaziat"]==0){
                                      echo'
                                      <a href="inc/cv0.php?cv0='.$row["cbi"].'" style="margin-right:0" class="btn btn-danger btn-xs" name="change0"><i class="fa fa-lock "></i></a>';
                                    }else{
                                      echo'
                                      <a href="inc/cv1.php?cv1='.$row["cbi"].'" style="margin-right:0" class="btn btn-success btn-xs" name="change1" ><i class="fa fa-unlock "></i></a>';
                                    }
                          ...
                          <div style="align-items: center;direction: ltr;text-align: center;" class="general-pagination group">
                            ...
                            echo '<a style="margin-right:2px;" href="show-comment-learn.php?pageid='.$i.'"><span class="badge bg-warning">'.$i.'</span></a>';
                             }
                            ?>
                          </div>
                      </div><!-- /content-panel -->
