...
<html lang="en">
  <head>
    ...
    <title>Mr.Developer | show_learns</title>
          ...
          <div id="sidebar"  class="nav-collapse ">
              ...
              <ul class="sidebar-menu" id="nav-accordion">
                  ...
                  <li class="sub-menu">
                      ...
                      <ul class="sub">
                          <li class="active"><a  href="show-learns.php?pageid=1">نمایش آموزش دوره ها</a></li>
          ...
          <section class="wrapper">
                  ...
                  <div class="col-md-12" style="margin-top: 20px;">
                      <div class="content-panel">
                          <h4></i>نمایش آموزش ها</h4><hr><table class="table table-striped table-advance table-hover">
                              <thead>
                              ...
                              <?php                 
                                ...
                                $show_pages=("SELECT * FROM `tutorails`,`tutorails_learn` WHERE tutorails_learn.tutorailsid=tutorails.id");  
                                $show_learns=("SELECT tutorails.id as tid,tutorails.title as tti,tutorails_learn.title as tlti,tutorails_learn.tutorailsid as tltu,tutorails_learn.id as tlid FROM `tutorails`,`tutorails_learn` WHERE tutorails_learn.tutorailsid=tutorails.id ORDER BY tlid DESC limit $start,$per_page");
                                $result=mysqli_query($sql,$show_learns);
                                $resu=mysqli_query($sql,$show_pages);
                                $coun = mysqli_num_rows($resu);
                                if(mysqli_num_rows($result) > 0){
                                while($row=mysqli_fetch_assoc($result)){
                                echo'<form method="post" action="inc/dltutorails_learn.php" id="frm"   onSubmit="return dl()">
                                      <tr style="font-family:roya;">
                                      <td>'.$row["tti"].'</a></td>
                                      <td>'.$row["tlti"].'</td>
                                      <td>
									                    <a href="edit-learns.php?learnid='.$row['tlid'].'" class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i></a>
                                      <input type="hidden" name="inputhidden" value='.$row["tlid"].'>
                          ...
                          <div style="align-items: center;direction: ltr;text-align: center;" class="general-pagination group">
                            ...
                            echo '<a style="margin-right:2px;" href="show-learns.php?pageid='.$i.'"><span class="badge bg-warning">'.$i.'</span></a>';
                             }
                            ?>
                          </div>
                      </div><!-- /content-panel -->
